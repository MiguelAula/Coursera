...
trait Solver extends GameDef {
  ...
  /*{
    @tailrec
    def loop(n: List[(Block,Move)], acc: LazyList[(Block, List[Move])] = LazyList()): LazyList[(Block, List[Move])] = n match {
      case Nil => acc.reverse
      case (block,move) :: rest => loop(rest,(block,move :: history) #:: acc)
    }
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    initial match {
      ...
      case (block, moves) #:: _ =>
        val more = for {
          nextNeighbor <- newNeighborsOnly(neighborsWithHistory(block, moves), explored)
          if !explored.contains(nextNeighbor._1)
        } yield nextNeighbor
  ...
  /*
    initial match {
      ...
      case (block, moves) #:: _ =>
        val next = for {
          nextNeighbor <- newNeighborsOnly(neighborsWithHistory(block, moves), explored)
        } yield from(LazyList(nextNeighbor), explored + block)
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] =
    for {
      ...
      if block.isStanding && block.b1 == goal
    } yield (block,moves)
