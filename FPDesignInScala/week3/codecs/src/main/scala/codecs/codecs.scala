...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] =
    Decoder.fromFunction { case n: Json.Num => if (n.value.isValidInt) Some(n.value.toInt) else None }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case l: Json.Arr =>
        @tailrec
        def traverse(jsL: List[Json], acc: List[A] = List()): Option[List[A]] =
          jsL match {
            case Nil => Some(acc.reverse)
            case x :: xs => decoder.decode(x) match {
              case None => None
              case Some(v) => traverse(xs,v :: acc)
            }
          }
...
trait ContactsCodecs {
  ...
  implicit val contactsEncoder: Encoder[Contacts] = ObjectEncoder.field("people")(Encoder.listEncoder[Person]).transform[Contacts](contacts => contacts.people)

  /** The corresponding decoder for `Contacts` */
  implicit val contactsDecoder: Decoder[Contacts] = Decoder.field("people")(Decoder.listDecoder[Person]).transform[Contacts](persons => Contacts(persons))
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //type xAndBool = (Int,Boolean)
    val xAndBool = ObjectEncoder.field[Int]("x").zip(ObjectEncoder.field[Boolean]("bool"))
    val listXAndBool = Encoder.listEncoder(xAndBool)
    println(listXAndBool.encode(List((1,true),(0,false))))

    //Contacts
    val contacts = List(Person("Pep",43),Person("Joan",32))
    println(renderJson(contacts))
  }
